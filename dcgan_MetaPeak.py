...
def generator_model(random_vector_size):
    # bulid the generator model, it is a model made up of UpSample and Convolution
    model = Sequential()
    model.add(Dense(input_dim=random_vector_size, output_dim=1024))
...
def train(BATCH_SIZE, epoch_num, random_vector_size):
    ...
    generator = generator_model(random_vector_size)
    ...
    noise = np.zeros((BATCH_SIZE, random_vector_size))
    ...
    for epoch in range(epoch_num):
        ...
        for index in range(batches_num):
            ...
            for i in range(BATCH_SIZE):
                noise[i, :] = np.random.uniform(-1, 1, random_vector_size)
...
def generate(BATCH_SIZE, random_vector_size, nice=False, ):
    generator = generator_model(random_vector_size)
    generator.compile(loss='binary_crossentropy', optimizer="SGD")
    ...
    if nice:
        ...
        noise = np.zeros((BATCH_SIZE*20, random_vector_size))
...
def get_args():
    ...
    parser.add_argument("--random_vector_size", type=int, default=100)
...
if __name__ == "__main__":
    ...
    if args.mode == "train":
        ...
        train(BATCH_SIZE=args.batch_size, epoch_num=args.epoch_num, random_vector_size=args.random_vector_size)
    elif args.mode == "generate":
        generate(BATCH_SIZE=args.batch_size, nice=args.nice, random_vector_size=args.random_vector_size)
